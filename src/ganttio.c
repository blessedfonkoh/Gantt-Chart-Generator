...
void displayGantt(Task tasks[10], int num_tasks)
{
    ...
    {
        printf("%s: from %ld to %ld ( depends on %ld: ", tasks[i].name, tasks[i].startMonth, tasks[i].endMonth, tasks[i].numDependencies);
        for (size_t j = 0; j < tasks[i].numDependencies; j++)
        {
            printf("%ld ", tasks[i].dependencies[j]);
        }
        printf(")\n");
    }
}